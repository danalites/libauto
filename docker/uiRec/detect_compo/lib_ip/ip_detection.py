    ...
    for i in range(0, row, step_h):
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                # ignore small area
                range_x, range_y = range_hv[0], range_hv[1]
                if component.width <= range_x[0] or component.width >= range_x[1] or component.height <= range_y[0] or component.height >= range_y[1]:
                    continue
