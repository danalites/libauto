...
def compo_detection(input_img_path, output_root, uied_params,
    ...
    uicompos = det.component_detection(binary, min_obj_area=int(uied_params['min-ele-area']), range_hv=range_hv)
    ...
    out = file.save_corners_json(pjoin(ip_root, name + '.json'), uicompos, print=True)
