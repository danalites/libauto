...
# https://raw.githubusercontent.com/AoiKuiyuyou/AoikHotkey/7a04f8fddee02df41ff436a696b5261ae588cb62/src/aoikhotkey/event_manager_macos.py
# https://gist.github.com/cosven/75523f1e970edcc4da8cf1908a9c1463
# https://stackoverflow.com/questions/11347862/system-wide-shortcut-for-mac-os-x
...
class EventManager(object):
    ...
    def __init__(self, callback):
        ...
        self.last_time = 0
        self.key_stroke_history = deque([], 10)
        self.callback = callback

    def detect_patterns(self):
        # example: ctrl+v => ctrl+v+v+ctrl (considering press+release)
        history = "+".join(self.key_stroke_history)
        print(history)

    def on_key_up(self, event):
        propagate = True
        now = time.perf_counter()
        timeout = float(now - self.last_time)
        self.last_time = now

        # clear queue if key stroke intervals too long
        if timeout > 1.5:
            self.key_stroke_history.clear()

        keyStr = event.__dict__.get("Key")
        self.key_stroke_history.append(keyStr)

        # LogWarning(f"[PRESS] {self.key_stroke_history}")
    ...
    def _tap_callback_inner(self, proxy, type_, event, refcon):
        ...
        # If is modifier key event (e.g., control)
